...
cy=(cx)';
 ...
 [Mc,Wbar,Qbar,peaks]=potentialMask(W,Q, I); 
...
figure;imshow(Q);colormap(jet); %%% fig 7
 ...
 function [Mc,Wbar,Qbar,peaks]=potentialMask(W,Q, I)
 tw=20;
 tq=0.5;
 Mc=zeros(353,253);
 Wbar=zeros(353,253);
 ...
 Wbar=Qbar.*Mc;
  peaks = houghpeaks(double(gray2ind(Wbar)), 40, 'threshold', double(ceil(0.3*max(Wbar(:)))) );
   figure ,plot(peaks) %9
    ...
    figure; imshow(I(3:353,3:253,:)); title('Result of Foerstner operator, r: foerstner regions, y: local max');
    hold on
