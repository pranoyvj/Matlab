...
nc=length(model.weight);% number of clusters
nx=length(trainVect(:,1));   % number of pixels row vector
    % trainvect :: nx ::xi or xj in this case
    
    
for i=1:nc
    for j=1:nx
    
        LnVectorProb(i,j)=log(model.weight(i))-...
            0.5*(log(det(squeeze(model.covar(i,:,:))))+...
            ((trainVect(j,:)-model.mean(i,:))*...
            (inv(squeeze(model.covar(i,:,:))))*...
            (trainVect(j,:)-model.mean(i,:))'));
       % create another array containing exponential values i.e alpha_c.p(xi|uc,sigmac)
        %tosum_denom(i,j)=exp(logvalue(i,j));
    end
...
%% to create denominator log(...)
LnVectorProb = CalcLnVectorProb(model, trainVect);

denom = log(sum(exp(LnVectorProb), 1));
...
function model = GmmMStep(model, trainVect, LnCompProb)
    nc=length(model.weight);% number of clusters
nx=length(trainVect(:,1));

exp_values=exp(LnCompProb);
...
for i=1:nc
    ...
    for j=1:nx
      ...
      mue=mue+trainVect(j,:)*exp_values(i,j);
    ...
    for j=1:nx
      ...
      sigma=sigma+((trainVect(j,:)-mue2)'*(trainVect(j,:)-mue2)*exp_values(i,j));  
        
    end
    sigma2=(1/np(i))*sigma;
    model.covar(i,:,:)=sigma2;
